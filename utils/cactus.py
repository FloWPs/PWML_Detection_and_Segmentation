...
class CrossViTMultiview(nn.Module): # MODIFIER PARAMS ICI
    def __init__(self, image_size, channels, num_classes, patch_size_small = 16, patch_size_large = 16, small_dim = 192,
                 large_dim = 192, small_depth = 4, large_depth = 4, cross_attn_depth = 1, multi_scale_enc_depth = 3,
    ...
    def forward(self, img):
        ...
        xs = self.to_patch_embedding_small(img[:, :3, :, :]) # MODIF FLORA (sagittal)
        b, n, _ = xs.shape
        ...
        xl = self.to_patch_embedding_large(img[:, 3:, :, :]) # MODIF FLORA (coronal)
        b, n, _ = xl.shape
